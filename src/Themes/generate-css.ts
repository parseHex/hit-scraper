...
export default function generateCSS(this: Themes, theme: ifc.ThemeName, mode?: string) {
	...
	// const ref = theme === 'random' ? this.randomize() : Settings.user.themes.colors[theme],
	const ref = Settings.user.themes.colors[theme];
	const _ms = mode === 'cell' || theme === 'classic';
	const cellFix = {
		row: (k: string) => `.${k} ` + (_ms ? '{background:' : 'a {color:') + ref[k] + '}',
		text: (k: string) => `.${k} {color:` + (_ms ? this.tune(ref.bodytable, ref[k]) : ref.bodytable) + '}',
		export: (k: string) => `.${k} button {color:` + (_ms ? this.tune(ref.export, ref[k]) : ref.export) + '}',
		vlink: (k: string) => `.${k} a:not(.static):visited {color:` + (_ms
			? this.tune(ref.vlink, ref[k])
			: ref.vlink) + '}'
	};
	let css = `
		body {color:${ref.defaultText}; background-color:${ref.background}}
